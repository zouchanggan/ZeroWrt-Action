...
# 版本设置
sed -i '/Firmware Version/ s|(L.isObject(boardinfo.release) ? boardinfo.release.description + '\'' / '\'' : '\'''\'') + (luciversion || '\'''\'')|'\''ZeroWrt-Super-$(date +%Y%m%d) / '\'' + (L.isObject(boardinfo.release) ? boardinfo.release.description : '\''OpenWrt Unknown Version'\'')|' /path/to/your/file.js feeds/luci/modules/luci-mod-status/htdocs/luci-static/resources/view/status/include/10_system.js
